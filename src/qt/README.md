This directory contains the BetchipQT graphical user interface (GUI). It uses the cross-platform framework [Qt](https://www1.qt.io/developers/).
...
Represents the main window of the Betchip UI.
...
ClientModel is used by the main application `betchipgui` and several models like `peertablemodel`.
...
Used to process BIP21 and BIP70 (see https://github.com/betchip/betchip/pull/11622) payment URI / requests. Also handles URI based application switching (e.g. when following a betchip:... link from a browser).
...
* UI elements like BetchipAmountField, which inherit from QWidget.
* `betchipstrings.cpp`: automatically generated
* `betchipunits.(h/cpp)`: BTCHP / mBTCHP / etc handling
* `callback.h`
...
* don't change `local/betchip_en.ts`; this happens [automatically](/doc/translation_process.md#writing-code-with-translations)
...
10. Start debugging with Qt Creator (you might need to the executable to "betchip-qt" under "Run", which is where you can also add command line arguments)

...
###   betchipcore.org and betchip.org and compares them.
...
WORKINGDIR="/tmp/betchip_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://betchipcore.org"
HOST2="https://betchip.org"
BASEDIR="/bin/"
VERSIONPREFIX="betchip-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'betchip-',
...
if [ $? -ne 0 ]; then
   echo "betchip.org failed to provide signature file, but betchipcore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "betchip.org and betchipcore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Betchip Core binary release signing key installed?"
   fi

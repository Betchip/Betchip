...
const char * const BETCHIP_CONF_FILENAME = "betchip.conf";
const char * const BETCHIP_PID_FILENAME = "betchipd.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "betchipd -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Betchip
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Betchip
    // Mac: ~/Library/Application Support/Betchip
    // Unix: ~/.betchip
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Betchip";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Betchip";
#else
    // Unix
    return pathRet / ".betchip";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", BETCHIP_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", BETCHIP_PID_FILENAME)));
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Betchip Core") == std::string::npos) {

...
The RPM spec file provided here is for Betchip-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://betchip.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Betchip git repository](https://github.com/betchip/betchip) but are not part of
...
Currently only `Source10` is used, for the example `betchip.conf` file.
...
in the betchip git repository. At present only an SVG version of the Betchip
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the betchip
...
needed when 0.12.1 is released, a proper fix is already in the Betchip git
master branch.
...
idea to build Betchip Core against a static version of that library compiled
...
The `betchip.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `betchip-server` package. At this point, most RPM based
...
The `betchip-server` package should have SELinux support. How to properly do
...
Betchip-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/betchip/). He has been
packaging Betchip for Fedora at least since 2012.
...
4. I renamed the `betchip` package that contains the Qt GUI to `betchip-core` as
that appears to be how the general population refers to it, in contrast to
`betchip-xt` or `betchip-classic`. I wanted to make sure the general population
